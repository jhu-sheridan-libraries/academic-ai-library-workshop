---
id: "04-author"
title: "Create One Skill for Multiple AI Tools"
...
steps:
  ...
  - index: 4
    ...
    type: "observe"
    instruction: "The exercise shows what remains the same and what changes when one source artifact is prepared for different AI tools. Check the answer against each item below."
    observe_items:
...
## Create One Skill for Multiple AI Tools
...
You author one **knowledge artifact** - a metadata block plus a written body - and the pipeline converts it into each tool's native format. Features a tool does not support may be omitted or rewritten, with a warning. You can see the whole idea by comparing one artifact's source with its compiled form, which is what this exercise does; running the compiler yourself is optional.
